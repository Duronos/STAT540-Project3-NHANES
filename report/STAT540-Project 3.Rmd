...
**Answer:** **Answer:**
The data come from the 2009–2012 National Health and Nutrition Examination Survey (NHANES), a cross-sectional survey of a nationally representative sample of U.S. adults. Each row is one person, and the 70+ columns record demographics (age, gender, income), health behaviors (physical activity, alcohol, smoking, sleep, sexual activity), and health outcomes and conditions such as diabetes, mental health, and general health status.
...
glimpse(people)
nrow(people)   # number of observations
...
**Answer:** In this confusion matrix, the model correctly identifies 6862 people without diabetes (true negatives) and 32 people with diabetes (true positives). It incorrectly labels 9 non-diabetic people as having diabetes (false positives) and misses 652 people who actually have diabetes (false negatives).
...
# pick out the patient with ID = 7 (row 7)
patient7 <- people %>% 
  slice(7)

new_x <- patient7

# using the derived tree model saved in mod_diabetes
predict(object = mod_diabetes, new_data = new_x, type = "class")

```

Was the prediction correct? Discuss, again, the tree model based on the graph from **part 5)** Yes, the prediction was correct: for patient 7 the tree predicts “No” for diabetes, and the actual Diabetes value in the data is also “No.” On the Part 5 graph, this patient falls in the large low-risk region of the plot (outside the older, very high-BMI blue rectangle), so the tree’s structure naturally assigns them to the “No diabetes” rectangle, which is consistent with the observed outcome.
...
```{r split_data_set}
set.seed(540)  # for reproducibility

# total rows
n <- nrow(people)

# indices for about 80% of the data
train_idx <- sample(1:n, size = floor(0.8 * n))

# training and testing data sets
people_train <- people[train_idx, ]
people_test  <- people[-train_idx, ]
...
null_accuracy <- max(diab_train$pct)
null_accuracy


```

What percentage of the sample has diabetes? What is the accuracy of the *null model*? About 9.25% of the training sample has diabetes.

A null model that always predicts “No diabetes” would have an accuracy of about 90.75%.
...
knn_fit <- train.kknn(
  formula = form,         # Diabetes ~ Age + Gender + BMI + HHIncome + PhysActive
  data    = people_train,
  kmax    = 15,           # try k = 1,...,15
  kernel  = "rectangular"
)

knn_fit   # shows chosen k and CV performance

# get predictions on the TEST set
knn_pred <- predict(knn_fit, newdata = people_test)

knn_results <- people_test %>%
  select(Diabetes) %>%
  mutate(Diabetes_knn = knn_pred)
...
# define a Naive Bayes model and fit on the training data
nb_fit <- naive_Bayes(mode = "classification") %>%
  set_engine("klaR") %>%          # engine for Naive Bayes
  fit(form, data = people_train)  # form was defined earlier

nb_fit

# predictions on the TEST set
nb_pred <- predict(nb_fit, new_data = people_test)

nb_results <- people_test %>%
  select(Diabetes) %>%
  bind_cols(nb_pred) %>%
  rename(Diabetes_nb = .pred_class)
...
logit_fit <- glm(form, data = people_train, family = binomial)

summary(logit_fit)

# predict on TEST data
logit_prob <- predict(logit_fit, newdata = people_test, type = "response")

# turn probabilities into class labels
logit_pred <- ifelse(logit_prob >= 0.5, "Yes", "No") %>%
  factor(levels = levels(people$Diabetes))

logit_results <- people_test %>%
  select(Diabetes) %>%
  mutate(Diabetes_logit = logit_pred)
...
accuracy(logit_results,
         truth    = Diabetes,
         estimate = Diabetes_logit)


```

#### (5 pts) Compare two of the two classifier using their accuracy.
Using the test set, the Naive Bayes classifier has an accuracy of about **91.7%**, while the logistic regression classifier has an accuracy of about **91.6%**. Naive Bayes is therefore *slightly* more accurate, but both models perform very similarly and only modestly better than the null model accuracy of about **90.8%**.
